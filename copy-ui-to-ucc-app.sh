...
# replace 'my-splunk-app:' with 'TA_CTIS_TAXII:' in all xml views in TA_CTIS_TAXII_ES_AR/package/default/data/ui/views
echo "Replacing app name in views XML files..."
find TA_CTIS_TAXII_ES_AR/package/default/data/ui/views -type f -name "*.xml" -exec sed -i '' 's/my-splunk-app/TA_CTIS_TAXII/g' {} \;

echo "Replacing app name in template HTML files..."
find TA_CTIS_TAXII_ES_AR/package/appserver/templates -type f -name "*.html" -exec sed -i '' 's/my-splunk-app/TA_CTIS_TAXII/g' {} \;
