from TA_CTIS_TAXII.package.bin.cim_to_stix import convert_to_stix_pattern
from TA_CTIS_TAXII.package.bin.cim_to_stix import IoCCategory
import pytest

# TODO: Change these tests to expect conversion from IoCCategory to STIX2 pattern

class TestOther:
    def test_should_throw_error_if_field_name_not_supported(self):
        with pytest.raises(NotImplementedError):
            convert_to_stix_pattern("random_field", "")
...
class TestFilePath:
    ...
    def test_filepath(self, field_name):
        pattern = convert_to_stix_pattern(field_name, r"C:\Windows\System32")
...
class TestFileHashes:
    def test_invalid_hash(self):
        with pytest.raises(NotImplementedError):
            convert_to_stix_pattern("file_hash", "abc")

    def test_md5(self):
        pattern = convert_to_stix_pattern("file_hash", "cead3f77f6cda6ec00f57d76c9a6879f")
        assert pattern == "[file:hashes.MD5 = 'cead3f77f6cda6ec00f57d76c9a6879f']"

    def test_sha1(self):
        pattern = convert_to_stix_pattern("file_hash", "aaf4c61ddcc5e8a2dabede0f3b482cd9aea9434d")
        assert pattern == "[file:hashes.'SHA-1' = 'aaf4c61ddcc5e8a2dabede0f3b482cd9aea9434d']"

    def test_sha256(self):
        pattern = convert_to_stix_pattern("file_hash",
...
class TestNetworkTraffic:
    def test_dest_ipv6(self):
        pattern = convert_to_stix_pattern("dest_ip", "2001:0db8:85a3:0000:0000:8a2e:0370:7334")
    ...
    def test_dest_ipv4_with_cidr_slash(self):
        pattern = convert_to_stix_pattern("dest_ip", "1.2.3.4/32")
    ...
    def test_src_ipv4_with_cidr_slash(self):
        pattern = convert_to_stix_pattern("src_ip", "1.2.3.4/32")
        assert pattern == "[network-traffic:src_ref.type = 'ipv4-addr' AND network-traffic:src_ref.value = '1.2.3.4/32']"

    def test_src_ipv6(self):
        pattern = convert_to_stix_pattern("src_ip", "2001:0db8:85a3:0000:0000:8a2e:0370:7334")
    ...
    def test_src_domain(self, field_name):
        pattern = convert_to_stix_pattern(field_name, "example.com")
    ...
    def test_should_throw_error_if_dest_ip_is_invalid(self):
        with pytest.raises(NotImplementedError):
            convert_to_stix_pattern("dest_ip", "abc")

    def test_dest_mac_address(self):
        pattern = convert_to_stix_pattern("dest_mac", "06:10:9f:eb:8f:14")
    ...
    def test_source_mac_address(self):
        pattern = convert_to_stix_pattern("src_mac", "06:10:9f:eb:8f:14")
