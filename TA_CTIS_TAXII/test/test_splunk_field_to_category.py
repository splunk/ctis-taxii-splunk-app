import pytest
from TA_CTIS_TAXII.package.bin.cim_to_stix import convert_splunk_field_to_category
...
class TestUnsupportedField:
    def test_unsupported_field(self):
        with pytest.raises(ValueError):
            convert_splunk_field_to_category("unsupported_field", "value")


class TestFileNameAndPath:
    def test_file_path(self):
        converted_category = convert_splunk_field_to_category("file_path", splunk_field_value="/path/to/file")
        assert converted_category == IoCCategory.FILE_PATH

    def test_file_name(self):
        converted_category = convert_splunk_field_to_category("file_name", splunk_field_value="file.exe")
...
class TestFileHashes:
    def test_invalid_file_hash(self):
        with pytest.raises(ValueError):
            convert_splunk_field_to_category("file_hash", splunk_field_value="invalid_hash")

    def test_md5(self):
        converted_category = convert_splunk_field_to_category("file_hash",
...
class TestNetworkTraffic:
    ...
    def test_dest_domain(self, field_name):
        converted_category = convert_splunk_field_to_category(field_name, splunk_field_value="example.com")
    ...
    def test_dest_ipv6(self):
        converted_category = convert_splunk_field_to_category("dest_ip", splunk_field_value=IPV6_ADDRESS)
        assert converted_category == IoCCategory.DESTINATION_IPV6

    def test_src_ipv6(self):
        converted_category = convert_splunk_field_to_category("src_ip", splunk_field_value=IPV6_ADDRESS)
        assert converted_category == IoCCategory.SOURCE_IPV6

    def test_dest_ipv4_with_cidr_slash(self):
        converted_category = convert_splunk_field_to_category("dest_ip", splunk_field_value=IPV4_CIDR)
        assert converted_category == IoCCategory.DESTINATION_IPV4

    def test_dest_ipv4(self):
        converted_category = convert_splunk_field_to_category("dest_ip", splunk_field_value=IPV4_ADDRESS)
        assert converted_category == IoCCategory.DESTINATION_IPV4

    def test_src_ipv4(self):
        converted_category = convert_splunk_field_to_category("src_ip", splunk_field_value=IPV4_ADDRESS)
        assert converted_category == IoCCategory.SOURCE_IPV4

    def test_src_ipv4_with_cidr_slash(self):
        converted_category = convert_splunk_field_to_category("src_ip", splunk_field_value=IPV4_CIDR)
        assert converted_category == IoCCategory.SOURCE_IPV4

    def test_should_validate_dest_ip(self):
        with pytest.raises(ValueError):
            convert_splunk_field_to_category("dest_ip", splunk_field_value="invalid_ip")

    def test_dest_mac_address(self):
        converted_category = convert_splunk_field_to_category("dest_mac", splunk_field_value=MAC_ADDRESS)
        assert converted_category == IoCCategory.DESTINATION_MAC_ADDRESS

    def test_source_mac_address(self):
        converted_category = convert_splunk_field_to_category("src_mac", splunk_field_value=MAC_ADDRESS)
